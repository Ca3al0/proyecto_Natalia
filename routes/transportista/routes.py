...
@transportista.route('/registro_fotografico/<int:pedido_id>', methods=['POST'])
@login_required
def guardar_registro_fotografico(pedido_id):
    pedido = Pedido.query.get_or_404(pedido_id)

    archivos_antes = request.files.getlist('fotos_antes')
    archivos_despues = request.files.getlist('fotos_despues')
    ...
    for archivo in archivos_despues:
        if archivo.filename != '':
            ...
            ruta = os.path.join(current_app.config['UPLOAD_FOLDER'], filename)
            archivo.save(ruta)
            nombres_despues.append(filename)

    registro = RegistroFotografico(
        ID_Pedido=pedido.ID_Pedido,
        fotos_antes=json.dumps(nombres_antes),
        fotos_despues=json.dumps(nombres_despues),
